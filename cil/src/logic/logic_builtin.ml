...
*)
  let boolean = { lt_name = "boolean"; lt_params = []; lt_ctors = None; } in
  let set = { lt_name = "set"; lt_params = ["elem"]; lt_ctors = None; } in
  let typetag = {lt_name = "typetag"; lt_params = []; lt_ctors = None; } in
  let sign = {lt_name = "sign"; lt_params = []; lt_ctors = None; } in
  let float_format = {lt_name = "float_format"; lt_params = []; lt_ctors = None; } in
  let rounding_mode = {lt_name = "rounding_mode"; lt_params = []; lt_ctors = None; } in
  ...
  List.iter
    (fun (typename, constrs) ->
	 ...
	 List.map
	   (fun cname ->
              let c =  { ctor_name = cname; ctor_type = typename; ctor_params = [] } in
	      Logic_env.add_builtin_logic_ctor cname c;
	      c)
	   constrs
       in
       typename.lt_ctors <- Some l)
    [ boolean, ["\\true"; "\\false"];
      sign , [ "\\Positive"; "\\Negative"] ;
      float_format, [ "\\Single"; "\\Double"; "\\Quad" ] ;
      rounding_mode, [ "\\Up"; "\\Down"; "\\ToZero"; "\\NearestAway"; "\\NearestEven" ];
    ];
  ...
  List.iter
    ...
    [ "\\is_finite", [], ["x", float_type] ;
      ...
      "\\no_overflow", [], ["f", float_format; "m", rounding_mode; "x", Lreal] ;
  ...
  List.iter
    ...
    [ "\\min", ["x",Linteger;"y",Linteger], Linteger ;
      ...
      (*"\\round_float", ["m", Linteger; "x", Lreal], float_type;*)
      "\\round_double", ["m", Linteger; "x", Lreal], double_type;
      (*"\\round_quad", ["m", Linteger; "x", Lreal], long_double_type;*)
    ]
