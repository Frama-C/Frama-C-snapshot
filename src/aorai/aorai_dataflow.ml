...
let compose_actions (fst,_,map1) (_,last,map2) =
  let map = 
...
let compose_states start_state end_state =
    ...
    try
      ...
      let composed_actions =
        Data_for_aorai.Aorai_state.Map.map 
          (fun elt -> compose_actions bindings elt) new_states
      in
...
module Computer(I: Init) = struct
  ...
  let computeFirstPredecessor stmt (s,loops) =
    let loops = 
      ...
      let res = 
        if is_loop then begin
          ...
          if Cil_datatype.Stmt.Set.mem stmt loops then begin
            ...
            Data_for_aorai.get_loop_init_state stmt
          end
        end else begin
  ...
  let stmt_can_reach = I.stmt_can_reach

  let doEdge _ _ t = t

  module StmtStartData = 
    Dataflow.StartData(struct type t = data let size = 17 end)
  ...
  if Data_for_aorai.isIgnoredFunction (Kernel_function.get_name kf) then
        ...
        struct 
          ...
          let loops =
            match Kernel_function.find_first_stmt kf with
              ...
              | _ -> Cil_datatype.Stmt.Set.empty
          let initial_state = (init_state, loops)
    ...
    let end_state = 
      if Kernel_function.is_definition kf then begin
        ...
        with Not_found ->
          Aorai_option.warning ~source:(fst (Cil_datatype.Kinstr.loc call_site))
...
struct
  ...
  let combineStmtStartData _ ~old st =
    if Data_for_aorai.included_state st old then None
    else Some (Data_for_aorai.merge_state old st)

  let combineSuccessors = Data_for_aorai.merge_state

  let doStmt s =
    match s.skind with
      | Return _ -> Dataflow.Done Reach.end_state
      | _ -> Dataflow.Default
        ...
        then Dataflow.Default (* we simply skip ignored functions. *)
          ...
          try
            ...
            let treat_one_state state map acc =
              ...
              let call_map = 
                Data_for_aorai.Aorai_state.Map.find state call_state in 
  ...
  let () =
    if Kernel_function.is_definition Reach.kf then begin
      let (all_stmts,_) =
        Dataflow.find_stmts (Kernel_function.get_definition Reach.kf)
      in
      List.iter
  ...
  let stmt_can_reach = Stmts_graph.stmt_can_reach Reach.kf
...
let filter_loop_states old_map restrict_map =
...
let backward_analysis_aux stack kf ret_state =
  ...
  (* recursive function: just attempt to filter wrt attainable current states *)
    ...
    let (all_stmts,sink_stmts) =
      Dataflow.find_stmts (Kernel_function.get_definition kf)
    in
    ...
    let treat_one_loop s =
      try
        ...
        (try
           ...
           let init = filter_loop_states init states in
           set_init_loop_state s init;
         with Not_found -> ());
        (try
           let inv = Data_for_aorai.get_loop_invariant_state s in
           let inv = filter_loop_states inv states in
