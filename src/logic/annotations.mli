...
val code_annotation_of_rooted: rooted_code_annotation -> code_annotation
(** Extract the code annotation from a rooted code annotation. *)
...
val code_annot: 
  ?emitter:Emitter.t -> 
  ?filter:(rooted_code_annotation -> bool) -> 
  stmt -> 
...
val iter_code_annot: 
  (Emitter.t -> rooted_code_annotation -> unit) -> stmt -> unit
(** Iter on each code annotation attached to the given statement. *)

val fold_code_annot: 
  (Emitter.t -> rooted_code_annotation -> 'a -> 'a) -> 
  stmt -> 
  'a -> 
  'a
...
val iter_all_code_annot:
  (stmt -> Emitter.t -> rooted_code_annotation -> unit) -> unit
(** Iter on each code annotation of the program. *)

val fold_all_code_annot:
  (stmt -> Emitter.t -> rooted_code_annotation -> 'a -> 'a) -> 
  'a ->
  'a
...
val add_code_annot: 
  Emitter.t -> ?kf:kernel_function -> stmt -> rooted_code_annotation -> unit
...
val code_annot_of_kf: kernel_function -> (stmt * rooted_code_annotation) list
...
val populate_spec_ref: (kernel_function -> funspec -> unit) ref
