...
module Make(X:sig val use_ctrl_dependencies: bool end) = struct
  ...
  let is_concerned_by_security _s =
    (* Julien: en fait, c'est peut-être plus difficile à coder que ce que je
       pensais dans la mesure où certains stmt ne sont pas dans l'ensemble
       [Stmts], par exemple les blocks (à vérifier cependant. *)
(*    compute ();
    let res =
      try
	Nodes.iter
