...
(* $Id: bycase_ai.ml,v 1.2 2008/10/02 13:33:29 uid588 Exp $ *)
...
let init_specification () =
  Globals.Functions.iter 
    (fun f -> 
       ...
       let post_st,post_tr = Ltl_utils.mk_empty_pre_or_post_bycase () in
...
class visit_propagating_pre_post_constraints_bycase (auto:Promelaast.buchautomata)  =
  ...
  let propagation_result = ref (Ltl_utils.mk_empty_pre_or_post_bycase ()) in
  (*                                                                         *)
  (***************************************************************************)
...
(** Propagates pre-condition to each statement, by following control flow. 
  ...
  let rec propagates_pre stmt_l (pre_st,pre_tr) =
	...
	pre_st,pre_tr
      with 
	| Not_found -> Ltl_utils.mk_empty_pre_or_post_bycase()
    in
	...
	let n_pre_st,n_pre_tr = Ltl_utils.double_bool_array_or_bycase (old_pre_st,old_pre_tr) pre in
    ...
    *)
    ...
    match stmt_l with
	  ...
	  if (Ltl_utils.double_bool_array_eq_bycase (pre_st,pre_tr) (Ltl_utils.mk_empty_pre_or_post_bycase())) then
  	    propagates_pre l (pre_st,pre_tr)
	  else
	    begin
	
	      (* Registering call context for future reinforcement of pre-condition *)
	      let (pre_call_st,pre_call_tr)=Ltl_utils.get_next_bycase vi.vname Promelaast.Call pre_st in

	      let (cur_pre_st,cur_pre_tr) = 
		if Hashtbl.mem functions_pre_usecase vi.vname 
		then Ltl_utils.double_bool_array_or_bycase (Hashtbl.find functions_pre_usecase vi.vname) (pre_call_st,pre_call_tr) 
		 ...
		 the operation is callable according to its post-condition. *)
	      (*let (post_call_st,post_call_tr) = (Data_for_ltl.get_func_post_bycase vi.vname) in *)
	      let (post_call_st,post_call_tr) = Ltl_utils.mk_forward_composition  pre_call_st (Data_for_ltl.get_func_post_bycase vi.vname) in
	  ...
	  let (post_st,post_tr) = Ltl_utils.double_bool_array_or_bycase post_block1 post_block2 in 
	  ...
	  if not (Ltl_utils.double_bool_array_eq_bycase (ref_pre_st,ref_pre_tr) (pre_st,pre_tr)) then 
	    begin
	  ...
	  (*consumes stmt_l *)
	  let _ = propagates_pre stmt_l (Ltl_utils.mk_empty_pre_or_post_bycase ()) in
	  (Ltl_utils.mk_empty_pre_or_post_bycase ())
	  ...
	  *)
	  ...
	  let block_pre  = ref( Ltl_utils.double_bool_array_or_bycase block_pre !block_post ) in


	  (* Fix-point computation *)
	  while not (Ltl_utils.double_bool_array_eq_bycase !old_post !block_post) do

	    old_post := !block_post;
	    block_post:=propagates_pre block.bstmts !block_pre;
	    block_pre :=Ltl_utils.double_bool_array_or_bycase !block_pre !block_post
	      
	  done;
	  
	  (* Finally : Post1  = Pre2 *)
	  let (loop_post_st,loop_post_tr) = !block_pre in (*!!!! INUTILE DEVRAIT ETRE A FALSE*)
  

	  (* Updating loop information *)
          Data_for_ltl.set_loop_ext_pre_bycase  (ref stmt) loop_pre;
	  Data_for_ltl.set_loop_ext_post_bycase (ref stmt) (loop_post_st,loop_post_tr);
	  Data_for_ltl.set_loop_int_pre_bycase  (ref stmt) !block_pre;
	  Data_for_ltl.set_loop_int_post_bycase (ref stmt) !block_post;
...
(** Propagates post-condition to each statement, by following control flow. 
  ...
  in let rec propagates_post stmt_l (post_st,post_tr) =
    ...
    let get_spec_of stmt_ref =  
      ...
      with Not_found -> Ltl_utils.mk_empty_spec_bycase() 
    in 
    ...
    let update_stmt_post stmt_ref (post_st,post_tr) =
      ...
      let new_post_st,new_post_tr = (Ltl_utils.double_bool_array_and_bycase (old_post_st,old_post_tr) (post_st,post_tr)) in
      Hashtbl.replace 
	stmts_spec
	stmt_ref
    ...
    *)
	...
	| ({skind=Instr(Call(_,Lval(Var(vi),_),_,_))} as stmt)::l ->
		...
		let (cur_post_st,cur_post_tr) = 
		  ...
		  then Ltl_utils.double_bool_array_or_bycase (Hashtbl.find functions_post_usecase vi.vname) (post_st,post_tr) 
		...
		(*let (pre_call_st,_) = Ltl_utils.mk_pre_or_post_bycase_from_pre_or_post (Data_for_ltl.get_func_pre vi.vname) in   *)
		let pre_call = 
		...
		let (cur_pre_st,cur_pre_tr) = Ltl_utils.double_bool_array_and_bycase cur_pre (pre_st,pre_tr) in
	  ...
	  let (pre_st,pre_tr) = Ltl_utils.double_bool_array_and_bycase cur_pre (pre_st,pre_tr) in
	  ...
	  let pre_blocks = Ltl_utils.double_bool_array_or_bycase pre_block1 pre_block2 in 
	  let (pre_st,pre_tr) = Ltl_utils.double_bool_array_and_bycase pre_blocks (pre_st,pre_tr) in
	  ...
	  if (not !second_computation_needed) 
	    ...
	    && (not (Ltl_utils.double_bool_array_eq_bycase (ref_pre_st,ref_pre_tr) (post_st,post_tr))) 
	  then
	  ...
	  *)
	  ...
	  let block_pre  = Data_for_ltl.get_loop_int_pre_bycase  (ref stmt) in
	  let block_post = Data_for_ltl.get_loop_int_post_bycase (ref stmt) in
	  ...
	  *)
	  ...
	  let block_pre  = propagates_post block.bstmts block_post in
          let block_pre  = Ltl_utils.double_bool_array_and_bycase block_pre !old_pre in
          let block_post = Ltl_utils.double_bool_array_and_bycase block_pre block_post in
	  ...
	  while not (Ltl_utils.double_bool_array_eq_bycase !old_pre !block_pre) do

	    old_pre := !block_pre ;
	    block_pre  := propagates_post block.bstmts !block_post ;
            block_pre  := Ltl_utils.double_bool_array_and_bycase !block_pre !old_pre ;
	    block_post := Ltl_utils.double_bool_array_and_bycase !block_pre !block_post 
	  ...
	  let loop_pre = Ltl_utils.double_bool_array_and_bycase loop_pre block_pre in



	  (* Updating loop information *)
          Data_for_ltl.set_loop_ext_pre_bycase  (ref stmt) loop_pre;
	  Data_for_ltl.set_loop_ext_post_bycase (ref stmt) loop_post;
	  Data_for_ltl.set_loop_int_pre_bycase  (ref stmt) block_pre;
	  Data_for_ltl.set_loop_int_post_bycase (ref stmt) block_post;
...
*)
    ...
    propagation_result := (Ltl_utils.mk_empty_pre_or_post_bycase ());
    ...
    let post     = Ltl_utils.double_bool_array_and_bycase post old_post in
...
*)
    ...
    let cur_pre = Ltl_utils.pre_flattening !cur_pre in
 ...
 *)
    ...
    if   (not (Ltl_utils.double_bool_array_eq_bycase starting_post ending_post) )
    then begin spec_modified:=true;  end;
  ...
  Globals.Functions.iter 
    (fun f -> 
	 ...
	 begin
	   ...
	   let pre = 
	     try Hashtbl.find functions_pre_usecase  name with Not_found -> (Ltl_utils.mk_empty_pre_or_post_bycase()) in 
	   let post = 
	     try Hashtbl.find functions_post_usecase name with Not_found -> (Ltl_utils.mk_empty_pre_or_post_bycase()) in

	   let pre = Ltl_utils.pre_flattening pre in
	   let post = Ltl_utils.post_pseudo_flattening post in
 
 
	   let cur_pre  = Ltl_utils.double_bool_array_and pre  old_pre  in 
	   let cur_post = Ltl_utils.double_bool_array_and_bycase post old_post in 


	   if   (not (Ltl_utils.double_bool_array_eq old_pre  cur_pre ) )  
	   then begin spec_modified:=true;  end;
	   if   (not (Ltl_utils.double_bool_array_eq_bycase old_post cur_post) )  
	   then begin spec_modified:=true;  end;
