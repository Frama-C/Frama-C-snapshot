...
let rec get_field_info_from_name my_list name =
  ...
  else  Ltl_to_acsl_option.fatal "no field found with name :%s" name
...
let get_new_offset my_host my_offset name=
  match my_host with
    | Cil_types.Var(var) -> 
          ...
          (*Cil_types.TComp(my_comp,_,_) = var_info.Cil_types.vtype in*)
           ...
           let get_field_from_offset my_offset = begin
             match my_offset with
               ...
               | _ ->  Ltl_to_acsl_option.fatal "support only struct no array wtih struct"
             end in
     ...
     | _ -> Ltl_to_acsl_option.fatal "NOT YET IMPLEMENTED : mem is not supported"
