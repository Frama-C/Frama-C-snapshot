...
object(self)
  ...
  method private create_prover p =
	...
	| VCS.NoResult -> ""
	| VCS.Valid -> "gtk-yes"
	| VCS.Failed -> "gtk-dialog-warning"
	| VCS.Unknown -> "gtk-no"
	| VCS.Timeout -> "gtk-cut"
	| VCS.Stepout -> "gtk-cut"
	| VCS.Invalid -> "gtk-no"
	| VCS.Computing _ -> "gtk-execute"
      in
      let render w =
	match Wpo.get_result w p with
	  | { VCS.verdict=VCS.NoResult } when p=VCS.Qed -> 
	      [ `PIXBUF(Gtk_helper.Icon.get Gtk_helper.Icon.Unmark) ]
	  | r -> [ `STOCK_ID (verdict r.VCS.verdict) ]
      in
      let title = VCS.name_of_prover p in
      let column = list#add_column_pixbuf ~title [] render in
