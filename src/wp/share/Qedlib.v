...
Tactic Notation "replace" constr(A) "->" constr(B) "by" tactic(T) :=
  let H := fresh in assert (H : A=B) by (T ; forward) ; rewrite -> H ; clear H.
Tactic Notation "replace" constr(A) "<-" constr(B) "by" tactic(T) :=
  let H := fresh in assert (H : A=B) by (T ; forward) ; rewrite <- H ; clear H.

Tactic Notation "replace" constr(A) "->" constr(B) "by" tactic(T) "in" hyp(G) :=
  let H := fresh in assert (H : A=B) by (T ; forward) ; rewrite -> H in G; clear H.
...
Proof.
  ...
  intro K. 
  replace access0 -> access1 by (apply extensionality).
  finish.
Qed.
...
Proof.
  ...
  repeat (replace (Zneg b) -> (- Zpos b) by auto with zarith) ;
