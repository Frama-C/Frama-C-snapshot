...
(** The [shift] of [nat -> A] functions. *)
Definition shift {A:Type} f i k : A := f (i + k).

(** bits of a positive with one more 1-bit. *)
Remark P_decomp_shift1: forall p: positive,
  shift (P_decomp p~1) 1 = P_decomp p.
Proof.
  intro p. extensionality k. unfold shift. auto.
Qed.

(** bits of a positive with one more 0-bit. *)
Remark P_decomp_shift0: forall p: positive,
  shift (P_decomp p~0) 1 = P_decomp p.
Proof.
  intro p. extensionality k. unfold shift. auto.
Qed.
...
Remark P_recomp_shift: forall (f: Zfc) (n i j: nat), 
  P_recomp n f (i+j) = P_recomp n (shift f i) j.
Proof.
  intros f n.
  induction n; intros i j; simpl; auto.
  unfold shift at 1.
  case_eqb BIT (f (i+j)); f_equal; 
    (replace (S(i+j)) -> (i + S j) by omega);
    apply IHn.
Qed.
...
Proof.
  ...
  unfold trailing.
  intros. unfold fnot.
  replace (N_decomp n k) -> false by (apply Zpos_decomp_trail; auto with arith).
  simpl. auto.
Qed.  
...
Proof.
  ...
  case_eqb SIGNX (bsign x); 
    (try replace (negb true) -> false by (by compute)); 
    (try replace (negb false) -> true by (by compute)); 
  ...
  (** x<0 , trailing ~x |y| false *)
    ...
    replace (negb true) -> false by auto.
    auto.
  (** x>0 , trailing x |x| false *)
    generalize (btrail x). rewrite SIGNX. auto.
  (** x>0 , trailing x |y| false *)
    generalize (btrail y). rewrite SIGNX in SEQ2. rewrite SEQ2. 
    replace (negb false) -> true by auto. rewrite BEQ1.
    unfold trailing. intros T k R. unfold fnot. rewrite T; auto with arith.
Qed.
...
Proof.
  ...
  (** x = Zneg p *)
    ...
    (** ZDEF *)
      ...
      replace u -> (-v) by (unfold u; unfold v; simpl; trivial).
      omega.
    (** cont. *)
...
Proof.
  ...
  (** POSITIVE SIGN *)
    ...
    apply N_decomp_recomp. 
    apply trailing_fnot. 
    replace (negb false) -> true by auto.
    rewrite fnot_inv.
    rewrite <- BSIGN.
    apply (btrail b).
  (** NEGATIVE SIGN *)
...
Proof.
  ...
  (** 0 <=two_power_nat n *)
    ...
    (** cont. *)
      ...
      replace (Nabs (2 * two_power_nat n)) -> (2 * Nabs (two_power_nat n))%N
        by by compute.
  ...
  (** 0 <=(two_power_nat n) - 1 *)
    ...
    (** cont. *)
      ...
      assert ((Nabs (2 * two_power_nat n - 1) = 2 * Nabs (two_power_nat n - 1) +1)%N) as EQ.
        ...
        replace (2 * two_power_nat n - 1) -> (2 * (two_power_nat n - 1) + 1) by omega.
...
Proof.
  ...
  replace (Nabs (Zpos p + Zpos p0)) -> (Npos p + Npos p0)%N by simpl.
  apply NxHpos_incr.
Qed.
...
Proof.
  ...
  case_leq 0 x; case_leq 0 (x + a); 
    intros; try (replace x -> 0 by omega); try (by compute).
  case_leq (-1) x; 
    intros; try (replace x -> (-1) by omega); try (by compute).
  assert (- (x + 1) >= 0) as X by omega.
  assert (- a >= 0) as A by omega.
  clear H; clear H0; clear H1; clear H2; clear H3. 
  replace (-(x+a+1)) -> (-(x+1) + -a) by omega.
...
Proof.
  ...
  (** positive *) 
    unfold ZxHpos. 
    replace (Nabs (Zpos p)) -> (Npos p) by (by simpl).
    replace (Zpos p) -> (Z_of_N (Npos p)) by (by simpl).
    induction p.
    (** 2p+1 *)
      simpl. simpl in IHp.
      replace (Zpos p~1) -> (2*(Zpos p) + 1)%Z by (auto with zarith).
      rewrite two_power_nat_S.
      omega.
    (** 2p *)
      simpl. simpl in IHp.
      replace (Zpos p~0) -> (2*Zpos p)%Z by (auto with zarith).
      rewrite two_power_nat_S.
      omega.
    (** one *)
      by compute.
...
Proof.
  ...
  (** base *)
    ...
    replace (two_power_nat 0) -> 1 by compute.
    omega.
  (** cont. *)
    rewrite two_power_nat_S. 
    induction Y0.
...
Proof.
  ...
  (** 0 <= z *)
    clear H.
    replace (two_power_nat n - 1) -> (z + ((two_power_nat n - 1) - z)) by omega.
  ...
  (** 0 > z *)
    rewrite <- (ZxHpos_sym z).
    unfold zlnot.
    replace (two_power_nat n - 1) -> (-(z+1) + (z+two_power_nat n)) by omega.
    pose (x := -(z+1)); fold x.
