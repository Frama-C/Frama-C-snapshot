...
let main (main_ui:Design.main_window_extension_points) =
  ...
  let module Configure_Hook = Hook.Make(struct end) in
  let launcher () = 
    let container = GPack.box `VERTICAL () 
    in
    Configure_Hook.extend (on_bool container "Enable function wise dependencies" Cmdline.ForceDeps.get Cmdline.ForceDeps.set);
    Configure_Hook.extend (on_bool container "Enable call wise dependencies" Cmdline.ForceCallDeps.get Cmdline.ForceCallDeps.set);

    "Dependencies", 
    Some container#coerce,
    Some Configure_Hook.apply,
    Some (fun () -> 
            if Cmdline.ForceDeps.get () then !Db.From.compute_all ();
