...
module Types : sig
  ...
  let add ~name ~pp_value_name ~dynamic ?(pp=no_pp) ?varname values =
    if !use_obj then begin
      ...
      let repr =
	Digest.string
	  (Marshal.to_string (name, values) [ Marshal.Closures ])
      in
  ...
  let repr_list ty =
    if !use_obj then List.map Obj.obj (find ty).repr
...
let register ~name ~value_name ?pp ?varname vl =
  if !use_obj then
    ...
    in
    let error () =
      invalid_arg ("Type.register: invalid representants for type " ^ name)
    in
    match vl with
    | [] -> error ()
    | v :: _ when Obj.tag (Obj.repr v) = Obj.closure_tag ->
	(* all the representants have the same types:
	   correct to check only one *)
	error ()
    | _ -> Types.add ~name ~pp_value_name ~dynamic ?pp ?varname vl
  else
    ""
...
let int = reg "int" ~pp:(fun _ fmt n -> Format.fprintf fmt "%d" n) [ 2 ]
let int32 =
  reg "int32" ~pp:(fun _ fmt n -> Format.fprintf fmt "%ld" n) [ Int32.zero ]
let int64 =
  reg "int64" ~pp:(fun _ fmt n -> Format.fprintf fmt "%Ld" n) [ Int64.zero ]
let nativeint =
  reg "nativeint" ~pp:(fun _ fmt n -> Format.fprintf fmt "%nd" n)
    [ Nativeint.zero ]
let float = reg "float" ~pp:(fun _ fmt f -> Format.fprintf fmt "%f" f) [ 4.3 ]
  ...
  let instantiate ty =
	...
	let repr =
	  Types.add
	    ...
	    ~dynamic:false
	    (List.map D.repr (Types.repr_list ty))
	in
...
module Polymorphic2(D:POLY2_INPUT) = struct
  ...
  let instantiate a b =
    if !use_obj then
      let key = a, b in
      try
	Hashtbl.find instances key
      with Not_found ->
	let repr_list =
	  List.fold_left
	    (fun acc r1 ->
	       List.fold_left
		 (fun acc r2 -> D.repr r1 r2 :: acc)
		 acc
		 (Types.repr_list b))
	    []
	    (Types.repr_list a)
  ...
  let instantiate ?label (a:'a) (b:'b t) =
	...
	let ty =
	  ...
	  Types.add
	    ...
	    ~dynamic:false
	    (List.map (fun r _ -> r) (Types.repr_list b))
	in
...
module Hashtbl_Arg: POLY2_INPUT with type ('a, 'b) t = ('a, 'b) Hashtbl.t =
struct
  type ('a, 'b) t = ('a, 'b) Hashtbl.t
  let name ty1 ty2 = "(" ^ name ty1 ^ ", " ^ name ty2 ^ ") Hashtbl.t"
  let value_name = "Type.Hashtbl"
  let repr k v = let h = Hashtbl.create 7 in Hashtbl.add h k v; h
  let pp _fa _fb _p _fmt _h =
    failwith "\"Type.Hashtbl.pp\" not yet implemented"
end

module Hashtbl : POLYMORPHIC2 with type ('a,'b) poly = ('a, 'b) Hashtbl.t =
  Polymorphic2(Hashtbl_Arg)

let hashtbl = Hashtbl.instantiate
