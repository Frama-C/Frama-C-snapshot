/* run.config
   LOG: csv.csv
   OPT: -no-autoload-plugins -load-module from,inout,report,scope,value -val-warn-copy-indeterminate=-main4 -val -remove-redundant-alarms -no-val-warn-on-alarms -value-msg-key=-alarm -then -report-csv @PTEST_RESULT@/csv.csv -report-no-proven -then -report-csv= -val-warn-on-alarms -slevel 1
