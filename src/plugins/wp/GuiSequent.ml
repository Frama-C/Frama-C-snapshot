...
class autofocus =
  object(self)
    ...
    method is_targetted e = autofocus && self#occurs_term e
...
class plang
  ...
  object(self)
    ...
    method! shareable e = autofocus#is_targetted e || super#shareable e
...
class focused (wtext : Wtext.text) =
  ...
  object(self)
    ...
    method private added_zone p q = targetted <- (p,q) :: targetted
    ...
    method goal w fmt =
      ...
      | GoalAnnot { VC_Annot.goal = goal } ->
          let sequent = Model.with_model w.po_model GOAL.compute_descr goal in
          Format.fprintf fmt "@\n@{<wp:clause>Goal@} %a:@\n" Wpo.pp_title w ;
          self#pp_sequent sequent fmt
