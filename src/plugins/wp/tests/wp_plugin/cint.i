...
// whatever the kernel options are (no RTE involved here). With +nat, have only signedness for types.
...
{
  return ~a; // no conversion in *all* models, including testcase 5 (nat)
}

//@ ensures R(\result,a);
short signed_downcast(int a)
{
  return (short) a; // identity *only* for testcases 3 & 5 (-warn-signed-downcast & nat)
}
...
{
  return (unsigned) a; // identity *only* for testcases 4 & 5 (-warn-unsigned-downcast & nat)
}

//@ ensures R(\result,a+b);
unsigned unsigned_overflow(unsigned a,unsigned b)
{
  return a+b; // identity *only* for testcases 2 & 5 (-warn-unsigned-overflow & nat)
}
